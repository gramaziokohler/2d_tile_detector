"""
https://github.com/gramaziokohler/clamp_controller/blob/master/src/visual_docking/aruco_markers.py
"""
import cv2
import numpy as np
...
def save_coefficients(mtx, dist, r_vecs, t_vecs, new_cam_mat, roi, path):
    """ Save the camera matrix and the distortion coefficients to given path/file. """
    cv_file = cv2.FileStorage(path, cv2.FILE_STORAGE_WRITE)
    cv_file.write("K", mtx)
    cv_file.write("D", dist)
    cv_file.write("R", r_vecs)
    cv_file.write("T", t_vecs)
    cv_file.write("NK", new_cam_mat)
